Shader "Projector/CausticsAlpha" {
	Properties {
	 ...
	 Subshader {
		...
		Pass {
			...
			fixed4 frag (v2f i) : COLOR {
				...
				float4 c = tex2Dlod (_MainTex, float4(fmod (i.uv, 1 / _Size)*_Size,0,dist/_LOD)); // project tiled texture, set lod.
				...
				return c * _Color *_Multiply * cAlpha; // apply final color
			}
			ENDCG
		}
	}
